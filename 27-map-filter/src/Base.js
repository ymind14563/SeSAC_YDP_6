...
export const Base = () => {
  ...
  /** 
   * - 모든 배열 뒤에 .map 가능
   * - array 자료의 갯수만큼 함수 안의 코드를 실행해줌
   * - return안에 있는 것들을 array로 반환해줌
   * - index => 반복문 돌 때마다 0 부터 1씩 증가
   * 
   * 
   * 
   ...
   * -- index : 현재 요소의 인덱스(위치) (선택사항)
   * -- array : 'map()'이 호출된 원본 배열 (선택사항)
   * 
   * - [thisArg] : callbackFunction 내부에서 this로 사용할 값을 지정 (선택사항)
