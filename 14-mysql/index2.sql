...
-- orders 테이블은 어떤 고객의 생일 정보를 알고 싶어도 customer 테이블 자체가 삭제되었기 때문에 알 수 없음.
-- pk-fk 연결된 테이블은 외래키가 설정된 테이블(참조테이블)이 먼저 삭제
-- 현 스키마에서는 (1) orders 테이블 삭제(drop) -> (2) customer 테이블 삭제(drop) 해야 customer 테이블 삭제 됨.
...
-- orders 테이블이 참조하는 customer 테이블에는 banana 라는 고객이 없기 때문에 오류가 발생한다. (Error Code: 1452)
...
-- customer 테이블에서 apple이 삭제 => orders 테이블은 어떻게 달라지는지? (apple 고객의 주문 정보가 함께 삭제된다.)
delete from customer where custid = 'apple'; -- orders 테이블 내 on delete cascade 설정
...
select * from customer where birth >= '1995-01-01' and birth <= '2000-12-31'; -- 상동, between과 같다.

-- 3. 집합
-- 주소가 서울 혹은 런던인 고객을 조회
select * from customer where addr in ('대한민국 서울', '영국 런던');
select * from customer where addr = '대한민국 서울' or addr = '영국 런던';

-- 주소가 서울 혹은 런던이 아닌 고객을 조회
...
select * from customer where custname like '_지'; -- 검색 결과 : 0 (이름 2글자에 2번째가 '지' 인 사람)


-- 고객 이름 세번째 글자가 '수'인 고객
select * from customer where custname like '__수%';
select * from customer where custname like '%수'; -- 세번째 글자를 의미하지 않고 마지막 글지만 '수'로 끝나는 사람
...
-- 6. order by : 정렬
select * from customer order by custname; -- default asc 순서
select * from customer order by custname desc; -- 역순

-- 주의) order by 절은 where 절 보다 뒤에 위치해야 함.
-- 2000년생 이후 출생자 중에서 주소를 기준으로 내림차순 검색
select * from customer where birth >= '2000-01-01' order by addr desc;
-- select * from customer order by addr descwhere birth >= '2000-01-01' -- 구문 오류 발생;
...
-- 7. limit : 개수 제한
-- 2000년생 이후 출생자 중에서 주소를 기준으로 내림차순 그리고 아이디를 기준으로 내림차순 검색하는ㄴ데 최초의 4개만 검색
select * from customer where birth >= '2001-01-01' order by addr desc, custid desc limit 4;
...
select * from orders; -- 전체 주문
select count(*) from orders; -- 전체 주문 개수

-- 주문 테이블에서 주문한 고객 수 (중복 없이)
select count(*) from customer; -- 전체 고객 수
select count(distinct custid) from orders; -- 총 회원 중 구매전환이 이루어진 고객 수
