// 비동기 처리
// 동기 처리라면 실행하고 기다리고 실행하고 기다리고 하나 실행할 때 몇십분이 걸릴 수도 있다.

// 1. setTimeout(code, delay)
// : delay(단위: ms) 시간 동안 기다리다가 code 함수를 실행

// 실행 결과 : // 1 -> 3 -> (2초 후) 2 순서로 출력된다.
// => js에서는 setTimeout 함수를 사용했을 때, 기다리지 않고 바로 다음 구문을 실행한다. (js의 특성)
...
// 목표: 콜솔로그 하나 찍고, 3초 뒤에 "고민 끝"이랑 상품명, 가격이 올바르게 출력되는 것.
let product, price;
function goMart () {
    console.log(`마트에 가서 어떤 음료를 살지 고민한다..`);
}
