...
// ex. body-parse 모듈
app.use(express.urlencoded({extended : true})); // urlencoded로 파싱된 body를 요청 -> post 요청으로 들어 오는 모든 형식의 데이터를 파싱.
...
app.post(`/postForm`, (req, res) => {
    ...
    // undefined -> body-parse 모듈 사용 안 했을 시
    // { id: 'apple', pw: '147258' } -> body-parse 모듈 사용 했을 시
    // res.send(`post 요청 성공`);
...
//res.render(뷰, 데이터) : 템플릿 엔진을 사용해서 서버츨에서 "동적으로" HTML 을 생성하고 클라이언트에게 응답
