...
// React Hook Form은 주로 언컨트롤드 컴포넌트를 사용함 (폼을 다루는 방식 2가지: 컨트롤드, 언컨트롤드)
// 폼 필드가 업데이트 될 때마다 전체 폼이 리랜더링되지 않도록 -> 최적화
// 필요할 때만 폼 플드의 상태를 업데이트하여 불필요한 랜더링을 줄이고, 결과적으로 앱의 성능을 향상시킨다.

// 일반 폼 작성 vs RHF 라이브러리 차이

// 일반 폼
// - 각 입력 필도의 상태를 useState로 관리해야함.
// - 모든 입력값의 상태를 관리하고, 값이 바뀔 때마다 전체 컴포넌트가 리랜더링 될 수 있음.
// - 검증 로직을 직접 다 작성해야 함.
...
// RHF 라이브러리
// - 코드의 간결화
// - 비제어 컴포넌트(= 언컨트롤드 컴포넌트) / 필요한 경우에만 리랜더링.
// - 다양한 검증 규칙 쉽게 설정.
// - 'handleSubmit' 함수 하나로 모든 폼 데이터를 쉽게 관리
// - 복잡하고 대규모 폼 양식을 작성할 때 사용.


// Q) 왜 ... 연산자를 사용하는가?
// React 컴포넌트에서 ... 연산자는 객체나 배열을 펼쳐서 각각의 요소나 속성을 개별적인 prop으로 전달할 때 사용
// 이 문법을 사용하면 코드가 간결해지고, 여러 개의 props를 한 번에 전달할 수 있음
